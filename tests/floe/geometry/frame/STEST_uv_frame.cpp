/*!
 * \file floe/geometry/frame/TEST_uv_frame.cpp
 * \brief TEST of UVFrame class.
 * \author Roland Denis
 */
...
int main()
{
    ...
    using Frame = fg::frame::UVFrame<Point>;
    using Circle = fg::Circle<Point>;

    const real pi = boost::math::constants::pi<real>();

    const Frame frame({1.0, 2.0}, {std::cos(pi/4), std::sin(pi/4)});
