...
TEST_CASE( "Test topology", "[topology]" ) {
    ...
    ToricTopology<point_type> T{x0, x1, y0, y1};
    ...
    REQUIRE( equal_points(g_list[0] - pt, {dx, -dy}));
    REQUIRE( equal_points(g_list[1] - pt, {dx, 0}));
    REQUIRE( equal_points(g_list[2] - pt, {dx, dy}));
    REQUIRE( equal_points(g_list[3] - pt, {0, dy}));
