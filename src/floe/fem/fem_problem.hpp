...
public :
    ...
    { // at the creation of a kinematic floe, the StaticFloe (and its mesh) do not exist yet. This function allows the initialisation of all afferent properties afterwards.
        m_floe = floe;
        m_mesh = m_floe->mesh();
    ...
    bool compute_stress_vector()
    {
        ...
        {
            B = computeB(iElem);
            sol_elem(0,0) = m_Solution.coeff(m_connect[iElem][0]*2, 0);
            sol_elem(1,0) = m_Solution.coeff(m_connect[iElem][0]*2+1, 0);
            sol_elem(2,0) = m_Solution.coeff(m_connect[iElem][1]*2, 0);
            sol_elem(3,0) = m_Solution.coeff(m_connect[iElem][1]*2+1, 0);
            sol_elem(4,0) = m_Solution.coeff(m_connect[iElem][2]*2, 0);
            sol_elem(5,0) = m_Solution.coeff(m_connect[iElem][2]*2+1, 0);
            sigma = H*B*sol_elem;
    ...
    bool compute_elementary_energies()
    {
        ...
        for (size_t iElem = 0; iElem < m_nE; iElem++)
        {
            ...
            e(0,0) = 0.0;
            real_type detJac = m_mesh.get_jacobian(iElem);
            Eigen::Matrix<real_type, 6, 1> u;
            u(0,0) = m_Solution.coeff(m_connect[iElem][0]*2,   0);
            u(1,0) = m_Solution.coeff(m_connect[iElem][0]*2+1, 0);
            u(2,0) = m_Solution.coeff(m_connect[iElem][1]*2,   0);
            u(3,0) = m_Solution.coeff(m_connect[iElem][1]*2+1, 0);
            u(4,0) = m_Solution.coeff(m_connect[iElem][2]*2,   0);
            u(5,0) = m_Solution.coeff(m_connect[iElem][2]*2+1, 0);
    ...
    std::vector<int> distribute_elements_on_both_sides_of(point_type a, point_type b)
    {
        ...
        {
            x0 = m_coordinates[iNode][0];
            y0 = m_coordinates[iNode][1];
        ...
        for (size_t iElem = 0; iElem < m_nE; iElem++)
        {
            if (node_sides[m_connect[iElem][0]] + node_sides[m_connect[iElem][1]] + node_sides[m_connect[iElem][2]] == 3) elem_sides[iElem] = 1;
            else if (node_sides[m_connect[iElem][0]] + node_sides[m_connect[iElem][1]] + node_sides[m_connect[iElem][2]] == -3) elem_sides[iElem] = -1;
        }
        return elem_sides;
    }
        ...
        for (size_t iElem = 0; iElem < m_nE; iElem++)
        {
            if (elem_sides[iElem] == 0)
                for (size_t iNode = 0; iNode < 3; iNode++)
                    nodes_on_the_fract.push_back(point_type(m_coordinates[m_connect[iElem][iNode]][0], m_coordinates[m_connect[iElem][iNode]][1]));
            else
...
FemProblem<TFloe>::assembleK()
{
    ...
    for (size_t iElem = 0 ; iElem < m_nE ; ++iElem)
    {
        ...
        detJac = m_mesh.get_jacobian(iElem);
        ...
        {
            size_t globalI = m_nDof*m_connect[iElem][i];
            for (unsigned int j = 0; j < m_nNodesPerElt ; j++ )
            {
                size_t globalJ = m_nDof*m_connect[iElem][j];
...
FemProblem<TFloe>::addContactDirichlet(std::vector<size_t> gamma_d, std::vector<point_type> values)
{
    ...
    for (size_t iDir = 0; iDir < n_dirichlet ; ++iDir)
    {
        // std::cout << "dirichlet point " << iDir << "/n_dirichlet ; " << std::endl;
        // std::cout << "value : " << values[iDir] << " ; locations : " << gamma_d[iDir] << std::endl;
        // std::cout << " Just checkin : values.size() = " << values.size() << " and we are seeking value " << iDir << std::endl;
        ...
        {
            if ((m_connect[iElem][0] == gamma_d[iDir]) || (m_connect[iElem][1] == gamma_d[iDir]) || (m_connect[iElem][2] == gamma_d[iDir]))
        ...
        else
        {
            ...
            {
                surr_nodes_temp.push_back(m_connect[surr_elements[iElem]][0]);
                surr_nodes_temp.push_back(m_connect[surr_elements[iElem]][1]);
                surr_nodes_temp.push_back(m_connect[surr_elements[iElem]][2]);
            }
...
FemProblem<TFloe>::computeB(size_t iElem) const
{
    ...
    double x0 = m_coordinates[m_connect[iElem][0]][0];
    double x1 = m_coordinates[m_connect[iElem][1]][0];
    double x2 = m_coordinates[m_connect[iElem][2]][0];
    double y0 = m_coordinates[m_connect[iElem][0]][1];
    double y1 = m_coordinates[m_connect[iElem][1]][1];
    double y2 = m_coordinates[m_connect[iElem][2]][1];
    // double detJac = m_floe->mesh().get_jacobian(iElem);
    double detJac = m_mesh.get_jacobian(iElem);
