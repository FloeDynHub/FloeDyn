...
#include "floe/lcp/jam_manager.hpp"                 // OPTIMJAM: dense-jam handling (see doc/optimjam.md)
...
public:
    ...
    //! Destructor (the OPTIMJAM #OPTIMJAM GS summary is printed by the JamManager member's own
    //! destructor, which runs right after this body — same output order as before).
    ~LCPManager(){
        if (m_nb_lcp)
    ...
    /*! OPTIMJAM: all the dense-jam machinery (routing, temporal freeze & probes, two-pass Gauss-
     *  Seidel solve, guard-rails, stats) lives in the JamManager — see jam_manager.hpp and
     *  doc/optimjam.md. The public API below is kept on LCPManager (thin delegation) so callers
     *  (simu_runner, problem) are unchanged. With set_optim_jam(false) (default), behaviour is
     *  identical to baseline. */
    inline void set_optim_jam(bool v) { m_jam.set_enabled(v); }
    inline bool optim_jam() const { return m_jam.enabled(); }
    inline void set_gs_params(int min_contacts, int max_iter, real_type rel_speed_max,
                              real_type eps, int stuck_N, int probe_K) {
        m_jam.set_params(min_contacts, max_iter, rel_speed_max, eps, stuck_N, probe_K);
    }
...
private:
    ...
    JamManager m_jam; //!< OPTIMJAM: dense-jam machinery (see jam_manager.hpp, doc/optimjam.md)
...
int LCPManager<T>::solve_contacts(TContactGraph& contact_graph, real_type time, real_type dt)
{
    ...
    for ( auto& subgraph : subgraphs )
    {
        ...
        // components (dense jams) are handled by the JamManager — temporal freeze of the blocked
        // floes + a two-pass Gauss-Seidel solve of the whole component. Returns false (component
        // left untouched and unfrozen) for everything else — impulsive collisions, small clusters,
        // free floes — or when its solver declines: the historical Lemke resolution below then runs.
        if ( m_jam.try_solve_component( subgraph ) ) continue;
