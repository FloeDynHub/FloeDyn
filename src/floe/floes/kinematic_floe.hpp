...
public:
    ...
    // Type traits
    using real_type = types::real_type;   //!< member alias (fracture code needs TFloe::real_type); matches the `using namespace types` name used in out-of-class defs
    using point_type = types::point_type;  //!< member alias (fracture code needs TFloe::point_type)
    ...
    // Dependent (injected-class-name) so FemProblem<Self> is NOT eagerly instantiated while
    // KinematicFloe is still incomplete — matches Silouane's deferred self-reference. See merge notes.
    using fem_problem_type = floe::fem::FemProblem<KinematicFloe>;
    // using fem_problem_type = floe::fem::FemProblem;
    // Predictor typed on THIS (base) floe, not on any Identifiable<...> MPI wrapper: keeps the type
    // consistent with descriptor()/fem_problem so PartialFloeGroup<Identifiable<KinematicFloe>> compiles.
    using fracture_predictor_type = floe::fem::FracturePredictor<KinematicFloe>;
    ...
    KinematicFloe() : m_geometry{nullptr}, m_floe{nullptr}, m_state{ {0,0}, 0, {0,0}, 0, {0,0}, true},
                      m_obstacle{false}, m_kinematic_mesh{}, m_total_impulse_received{0}, m_fem_problem{this}, m_fem_problem_is_set{false}, m_min_caliper_diameter{0}, m_max_caliper_diameter{0}, m_mean_caliper_diameter{0} {}

    KinematicFloe(static_floe_type new_static_floe) : m_geometry{nullptr}, m_floe{new_static_floe}, m_state{ {0,0}, 0, {0,0}, 0, {0,0}, true},
                      m_obstacle{false}, m_kinematic_mesh{}, m_total_impulse_received{0}, m_fem_problem_is_set{false}, m_min_caliper_diameter{0}, m_max_caliper_diameter{0}, m_mean_caliper_diameter{0} {}
