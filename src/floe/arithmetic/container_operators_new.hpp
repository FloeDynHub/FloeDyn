...
// template< typename T,
// 	class = typename std::enable_if_t< ! std::is_fundamental<T>::value > >
// struct value_type
// {
// 	typedef typename T::value_type type; //!< We can probably also use decltype(T()[0])
// };

template<typename T, class = std::enable_if_t< ! std::is_fundamental<T>::value > >
using value_type = decltype(T()[0]);
...
// struct activate : std::enable_if_t< is_activated<T>::value , Return > {};

template< class T, class Return >
using activate = std::enable_if_t< is_activated<T>::value , Return >;
...
value_type<T>
reduce( const T & lhs, Operator op, value_type<T> init ) {
 ...
 */
template< class T >
activate<T, value_type<T> >
sum( const T & arg ) {
	return tools::reduce( arg, std::plus<value_type<T>>(), value_type<T>(0) );
}
// template< class T >
// activate<T, value_type<T> >
// sum( const T & arg ) {
...
activate<T, value_type<T> >
prod( const T & arg ) {
	return tools::reduce( arg, std::multiplies<value_type<T>>(), value_type<T>(1) );
}
